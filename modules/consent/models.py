...
class ConsentManager:
    ...
    # ========================================================================
    # PHASE 1: CONSENT LEVEL IMMUTABILITY - NEW METHODS
    # ========================================================================
    
    def set_consent_level_immutable(self, case_id: str, level: ConsentLevel, reason: str = "") -> bool:
        """
        Set consent level with immutability lock.
        Once set, cannot be changed without explicit unlock.
        
        Args:
            case_id: Case ID
            level: ConsentLevel to set
            reason: Reason for setting this level (for audit trail)
        
        Returns:
            True if set successfully, False if already locked
        """
        session = self.sessions.get(case_id)
        if not session:
            logger.warning(f"Cannot set consent level: session {case_id} not found")
            return False
        
        # Check if already locked
        if getattr(session, '_consent_level_locked', False):
            logger.warning(f"Consent level for {case_id} is locked and cannot be changed")
            return False
        
        # Set the level
        session.level = level
        session._consent_level_locked = True
        session._consent_level_set_at = datetime.now().isoformat()
        
        # Log to audit trail
        audit_entry = {
            'timestamp': datetime.now().isoformat(),
            'action': 'CONSENT_LEVEL_SET_IMMUTABLE',
            'level': level.name,
            'reason': reason,
            'locked': True
        }
        self._append_audit_log(case_id, audit_entry)
        self._write_consent_snapshot(case_id)
        
        logger.info(f"✅ Consent level for {case_id} set to {level.name} (IMMUTABLE) - Reason: {reason}")
        return True
    
    def get_consent_level(self, case_id: str) -> Optional[ConsentLevel]:
        """
        Get the current consent level for a case.
        Returns the immutable level that was set.
        
        Args:
            case_id: Case ID
        
        Returns:
            ConsentLevel or None if not set
        """
        session = self.sessions.get(case_id)
        if not session:
            return None
    ...
    def is_consent_level_locked(self, case_id: str) -> bool:
        """
        Check if consent level is locked (immutable).
        ...
        if not session:
            return False
        return getattr(session, '_consent_level_locked', False)
    
    def get_consent_level_info(self, case_id: str) -> Dict[str, Any]:
        """
        Get detailed consent level information.
        
        Args:
            case_id: Case ID
        
        Returns:
            Dict with level, locked status, and set time
        """
        session = self.sessions.get(case_id)
        if not session:
            return {'level': None, 'locked': False, 'set_at': None}
        
        # Get scope name from EXTRACTION_SCOPES
        scope_name = 'Unknown'
        if session.level and session.level in EXTRACTION_SCOPES:
            scope_obj = EXTRACTION_SCOPES[session.level]
            scope_name = scope_obj.name if hasattr(scope_obj, 'name') else 'Unknown'
