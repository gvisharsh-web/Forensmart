...
from modules.consent.portal import ConsentAuditTrail, ConsentPortalEnhancer  # NEW: Integrated consent portal
...
# ========================================================================
# PHASE 2: MODULE REQUIREMENTS TABLE - CONSENT LEVEL HIERARCHY
# ========================================================================
# Define minimum consent level required for each module
# Hierarchy: NONE=0 < BASIC=1 < STANDARD=2 < LEGAL=3 < FULL=4
MODULE_MIN_LEVELS = {
    'device_info': ConsentLevel.NONE,      # No consent needed
    'communications': ConsentLevel.LEGAL,  # Requires LEGAL (value=4)
    'location': ConsentLevel.STANDARD,     # Requires STANDARD (value=2)
    'security': ConsentLevel.STANDARD,     # Requires STANDARD (value=2)
    'media': ConsentLevel.STANDARD,        # Requires STANDARD (value=2)
    'system': ConsentLevel.BASIC,          # Requires BASIC (value=1)
}
...
class DeviceInfoExtractor(ExtractionModule):
    ...
    def extract(self, device_id: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if not device_id:
                raise ValueError("Device ID is empty or invalid")
...
class CommunicationExtractor(ExtractionModule):
    ...
    def extract(self, device_id: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # NEW CONSENT SYSTEM: Check if consent level is sufficient
            # Communications requires LEGAL consent (value 4)
            if consent_manager and case_id:
                session = consent_manager.get_session(case_id)
                if session and session.level:
                    from modules.extraction.orchestrator import MODULE_MIN_LEVELS
                    min_level = MODULE_MIN_LEVELS.get('communications')
                    
                    # Check numeric consent level
                    if session.level.value < min_level.value:
                        logger.warning(f"Communications blocked: {session.level.name} ({session.level.value}) < {min_level.name} ({min_level.value})")
                        return {
                            'status': 'messaging_consent_denied',
                            'message': f'Communications requires {min_level.name} consent (current: {session.level.name})',
                            'required_level': min_level.name,
                            'current_level': session.level.name
                        }
                    
                    logger.info(f"Communications consent check passed: {session.level.name} >= {min_level.name}")
                else:
                    logger.warning("No session or consent level found for communications")
                    return {
                        'status': 'messaging_consent_denied',
...
class LocationExtractor(ExtractionModule):
    ...
    def __init__(self):
        super().__init__("Location", "GPS coordinates, cell tower data, and location history")
    ...
    def extract(self, device_id: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # NEW CONSENT SYSTEM: Check if consent level is sufficient
            # Location requires STANDARD consent (value 2)
            if consent_manager and case_id:
                session = consent_manager.get_session(case_id)
                if session and session.level:
                    min_level = MODULE_MIN_LEVELS.get('location')
                    
                    # Check numeric consent level
                    if session.level.value < min_level.value:
                        logger.warning(f"Location blocked: {session.level.name} ({session.level.value}) < {min_level.name} ({min_level.value})")
                        return {
                            'status': 'location_consent_denied',
                            'message': f'Location requires {min_level.name} consent (current: {session.level.name})',
                            'required_level': min_level.name,
                            'current_level': session.level.name
                        }
                    
                    logger.info(f"Location consent check passed: {session.level.name} >= {min_level.name}")
                else:
                    logger.warning("No session or consent level found for location")
                    return {
                        'status': 'location_consent_denied',
                        'message': 'No consent session found'
                    }
        except Exception as e:
            logger.error(f"Location consent check failed: {e}", exc_info=True)
            return {'status': 'error', 'error': f'Location consent check error: {e}'}
...
class SecurityExtractor(ExtractionModule):
    ...
    def __init__(self):
        super().__init__("Security", "Password strength, authentication methods, and security settings")
    ...
    def extract(self, device_id: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # NEW CONSENT SYSTEM: Check if consent level is sufficient
            # Security requires STANDARD consent (value 2)
            if consent_manager and case_id:
                session = consent_manager.get_session(case_id)
                if session and session.level:
                    min_level = MODULE_MIN_LEVELS.get('security')
                    
                    # Check numeric consent level
                    if session.level.value < min_level.value:
                        logger.warning(f"Security blocked: {session.level.name} ({session.level.value}) < {min_level.name} ({min_level.value})")
                        return {
                            'status': 'security_consent_denied',
                            'message': f'Security requires {min_level.name} consent (current: {session.level.name})',
                            'required_level': min_level.name,
                            'current_level': session.level.name
                        }
                    
                    logger.info(f"Security consent check passed: {session.level.name} >= {min_level.name}")
                else:
                    logger.warning("No session or consent level found for security")
                    return {
                        'status': 'security_consent_denied',
                        'message': 'No consent session found'
                    }
        except Exception as e:
            logger.error(f"Security consent check failed: {e}", exc_info=True)
            return {'status': 'error', 'error': f'Security consent check error: {e}'}
...
class MediaExtractor(ExtractionModule):
    ...
    def __init__(self):
        super().__init__("Media", "Photos, videos, audio files, and thumbnails")
    ...
    def extract(self, device_id: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # NEW CONSENT SYSTEM: Check if consent level is sufficient
            # Media requires STANDARD consent (value 2)
            if consent_manager and case_id:
                session = consent_manager.get_session(case_id)
                if session and session.level:
                    min_level = MODULE_MIN_LEVELS.get('media')
                    
                    # Check numeric consent level
                    if session.level.value < min_level.value:
                        logger.warning(f"Media blocked: {session.level.name} ({session.level.value}) < {min_level.name} ({min_level.value})")
                        return {
                            'status': 'media_consent_denied',
                            'message': f'Media requires {min_level.name} consent (current: {session.level.name})',
                            'required_level': min_level.name,
                            'current_level': session.level.name
                        }
                    
                    logger.info(f"Media consent check passed: {session.level.name} >= {min_level.name}")
                else:
                    logger.warning("No session or consent level found for media")
                    return {
                        'status': 'media_consent_denied',
                        'message': 'No consent session found'
                    }
        except Exception as e:
            logger.error(f"Media consent check failed: {e}", exc_info=True)
            return {'status': 'error', 'error': f'Media consent check error: {e}'}
...
class SystemExtractor(ExtractionModule):
    ...
    def __init__(self):
        super().__init__("System", "System logs, configuration, and diagnostic information")
    ...
    def extract(self, device_id: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # NEW CONSENT SYSTEM: Check if consent level is sufficient
            # System requires BASIC consent (value 1)
            if consent_manager and case_id:
                session = consent_manager.get_session(case_id)
                if session and session.level:
                    min_level = MODULE_MIN_LEVELS.get('system')
                    
                    # Check numeric consent level
                    if session.level.value < min_level.value:
                        logger.warning(f"System blocked: {session.level.name} ({session.level.value}) < {min_level.name} ({min_level.value})")
                        return {
                            'status': 'system_consent_denied',
                            'message': f'System requires {min_level.name} consent (current: {session.level.name})',
                            'required_level': min_level.name,
                            'current_level': session.level.name
                        }
                    
                    logger.info(f"System consent check passed: {session.level.name} >= {min_level.name}")
                else:
                    logger.warning("No session or consent level found for system")
                    return {
                        'status': 'system_consent_denied',
                        'message': 'No consent session found'
                    }
        except Exception as e:
            logger.error(f"System consent check failed: {e}", exc_info=True)
            return {'status': 'error', 'error': f'System consent check error: {e}'}
...
class DataExtractionOrchestrator:
    ...
    def extract_all_data(
        ...
        try:
            ...
            # ====================================================================
            # HYBRID APPROVAL CHECK - Online (Supabase) + Offline (File) Fallback
            # ====================================================================
            try:
                from modules.approval.approval_manager_hybrid import get_hybrid_approval_manager
                hybrid_manager = get_hybrid_approval_manager()
                approval_check = hybrid_manager.check_approval_with_fallback(case_id)
                
                logger.info(f"✅ Approval check for {case_id}: status={approval_check['status']}, source={approval_check['source']}")
            except ImportError:
                logger.warning("Hybrid approval manager not available, using fallback")
                approval_check = ApprovalManager.check_approval_with_fallback(case_id)
                logger.info(f"Approval check for {case_id}: status={approval_check['status']}, source={approval_check['source']}")
            except Exception as e:
                logger.error(f"Error during approval check: {e}", exc_info=True)
                results['status'] = 'error'
                results['errors'].append(f"Approval check failed: {str(e)}")
                return results
            
            # Check approval result
            if not approval_check.get('approved', False):
                status = approval_check.get('status', 'pending')
                
                if status == 'denied':
                    message = 'Nominee denied the extraction request'
                    results['status'] = 'denied'
                    logger.warning(f"❌ Extraction DENIED for {case_id}")
                else:
                    message = 'Awaiting nominee approval for extraction'
                    results['status'] = 'pending_approval'
                    logger.info(f"⏳ Extraction PENDING for {case_id}")
                
                results['errors'].append(message)
                results['approval_source'] = approval_check.get('source', 'unknown')
                results['approval_error'] = approval_check.get('error', '')
                return results
            
            # ====================================================================
            # APPROVAL GRANTED - Proceed with extraction
            # ====================================================================
            logger.info(f"✅ APPROVAL GRANTED for {case_id}")
            results['approval_source'] = approval_check.get('source', 'unknown')
            results['approved_by'] = approval_check.get('nominee_name', '')
            results['approval_timestamp'] = approval_check.get('timestamp', '')
            logger.info(f"   Source: {results['approval_source']}")
            logger.info(f"   Approved by: {results['approved_by']}")
            ...
            # NEW APPROVAL SYSTEM: Skip old OTP verification, use new approval instead
            # The approval was already checked above (line 1227-1242)
            # If we reach here, approval is already verified
            logger.info(f"✅ Approval verified for {case_id} - proceeding with extraction")
            logger.info(f"Consent level: {session.level} - extraction authorized")
                ...
                while retry_count <= max_retries:
                    try:
                        ...
                        if progress_callback:
                            ...
                            progress_callback(
                                progress,
                                f"{status} - Extracting {module_name}...",
                        ...
                        # Pass consent_manager to ALL modules that need consent checks
                        if module_name in ['communications', 'location', 'security', 'media', 'system']:
            ...
            # Add consent level to results
            results['consent_level'] = session.level.name if session and session.level else None
    ...
    # ========================================================================
    # PHASE 2: MODULE CONSENT CHECKING - HELPER FUNCTIONS
    # ========================================================================
    
    def check_module_consent(self, module_name: str, current_level: ConsentLevel) -> tuple[bool, str]:
        """
        Check if current consent level allows a module to run.
        
        Args:
            module_name: Name of the module (e.g., 'communications')
            current_level: Current ConsentLevel
        
        Returns:
            (allowed: bool, message: str)
        """
        if module_name not in MODULE_MIN_LEVELS:
            return True, f"Module {module_name} has no consent requirement"
        
        min_level = MODULE_MIN_LEVELS[module_name]
        
        # Compare numeric values: NONE=0, BASIC=1, STANDARD=2, LEGAL=3, FULL=4
        if current_level.value >= min_level.value:
            return True, f"✅ {module_name} allowed (requires {min_level.name}, current {current_level.name})"
        else:
            return False, f"❌ {module_name} blocked: requires {min_level.name} consent (current {current_level.name})"
