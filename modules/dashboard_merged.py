...
# --- Modern UI Imports ---
from modules.extraction.ui import ( # pyright: ignore[reportMissingImports]
    render_extraction_tab,
    get_extraction_ui_manager
)
# --- End Modern UI Imports ---

from modules.consent.models import ConsentManager, ConsentLevel, ConsentSession # pyright: ignore[reportMissingImports]
from modules.extraction.orchestrator import DataExtractionOrchestrator # pyright: ignore[reportMissingImports]
from modules.shared.unified_error_system import handle_error, validate_before_operation, get_error_stats # pyright: ignore[reportMissingImports]
from modules.shared.utils import ( # pyright: ignore[reportMissingImports]
    ResultsRepository,
    MediaManifest,
...
from modules.storage.manager import StorageManager, StorageAnalytics # pyright: ignore[reportMissingImports]
from modules.storage.ui import render_storage_dashboard # pyright: ignore[reportMissingImports]
from modules.shared.error_checker import ErrorChecker # pyright: ignore[reportMissingImports]
...
from modules.consent.portal import ConsentPortalEnhancer, ConsentAuditTrail, ConsentPortalLogger # pyright: ignore[reportMissingImports]
...
# ============================================================================
# TIER 2 FIX #6: REDIRECT LOGGING HELPER FUNCTION
# ============================================================================
def _log_redirect_event(case_id: str, source: str = 'approval_portal', status: str = 'success', **kwargs) -> bool:
    """Log redirect events to audit trail for debugging and compliance."""
    try:
        redirect_log_dir = Path('audit/redirects')
        redirect_log_dir.mkdir(parents=True, exist_ok=True)
        
        # Create redirect log entry
        redirect_entry = {
            'case_id': case_id,
        ...
        # Append to redirect history file
        history_file = redirect_log_dir / 'redirect_history.json'
        history = []
        ...
        # Also save individual case redirect log
        case_redirect_file = redirect_log_dir / f'{case_id}_redirect.json'
        case_redirect_file.write_text(json.dumps(redirect_entry, indent=2), encoding='utf-8')
        
        return True
    except Exception as e:
        st.warning(f"⚠️ Failed to log redirect event: {str(e)}")
        return False
...
def render_investigator_view(orchestrator: DataExtractionOrchestrator):
    ...
    # Determine which tab should be selected based on nav state
    default_tab_index = 0
    ...
    with tab_extraction:
        ...
        if case_id:
            # ====================================================================
            # TIER 1 FIX #5: DEVICE VALIDATION
            # ====================================================================
            ...
            if is_mobile:
                ...
                st.info("💻 **Desktop Device Detected** - Full extraction features available")
            
            # ====================================================================
            # TIER 2 FIX #9: EXTRACTION TYPE VALIDATION
            # ====================================================================
            ...
            # Check if auto-extract was triggered from approval redirect
    ...
    with tab_diagnostics:
        ...
        # ====================================================================
        # TIER 2 FIX #10: REDIRECT HISTORY VIEWER
        # ====================================================================
        ...
        if redirect_history_file.exists():
            try:
                ...
                if redirect_history:
                    st.success(f"✅ {len(redirect_history)} redirect events logged")
                    
                    # Filter options
                    col1, col2, col3 = st.columns(3)
                    with col1:
                        filter_case = st.text_input("Filter by Case ID", "")
                    with col2:
                        filter_status = st.selectbox("Filter by Status", ["All", "success", "failed", "approval_changed"])
                    with col3:
                        filter_source = st.selectbox("Filter by Source", ["All", "approval_portal", "approval_polling"])
                    
                    # Apply filters
                    filtered_history = redirect_history
                    
                    if filter_case:
                        filtered_history = [h for h in filtered_history if filter_case.lower() in h.get('case_id', '').lower()]
                    
                    if filter_status != "All":
                        filtered_history = [h for h in filtered_history if h.get('status') == filter_status]
                    
                    if filter_source != "All":
                        filtered_history = [h for h in filtered_history if h.get('source') == filter_source]
                    
                    # Display filtered results
                    st.write(f"Showing {len(filtered_history)} of {len(redirect_history)} events")
                    
                    # Show as expandable items
                    for i, event in enumerate(reversed(filtered_history[-20:])):  # Show last 20
                        with st.expander(f"📍 {event.get('case_id', 'N/A')} - {event.get('timestamp', 'N/A')}"):
                            col1, col2 = st.columns(2)
                            with col1:
                                st.write(f"**Status:** {event.get('status', 'N/A')}")
                                st.write(f"**Source:** {event.get('source', 'N/A')}")
                                st.write(f"**Extraction Type:** {event.get('extraction_type', 'N/A')}")
                            with col2:
                                st.write(f"**Device ID:** {event.get('device_id', 'N/A')}")
                                st.write(f"**Approval Status:** {event.get('approval_status', 'N/A')}")
                                st.write(f"**User Agent:** {event.get('user_agent', 'N/A')[:50]}...")
                    
                    # Export option
                    if st.button("📥 Export Redirect History as JSON"):
                        st.download_button(
                            label="Download JSON",
                            data=json.dumps(filtered_history, indent=2),
                            file_name=f"redirect_history_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
                            mime="application/json"
                        )
                else:
