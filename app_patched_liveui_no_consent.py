# DEV/TEST BUILD — Consent bypass enabled
# WARNING: This file forces extraction consent and ADB authorization flags for UI testing only.
...
def get_adb_devices():
    ...
    try:
        out = subprocess.check_output(["adb","devices","-l"], stderr=subprocess.STDOUT).decode(errors='ignore')
    except Exception as e:
        return {"error": str(e)}
...
def require_adb_ui(st):
    ...
    if dev['state'] != "device":
        st.session_state.adb_consent_ok = False
        st.error("Device not authorized for ADB. On the device, accept the RSA prompt and re-run.")
        if st.button("Re-check device"):
            pass
    else:
        ...
        if not st.session_state.extraction_consent_given:
            agree = st.checkbox("I confirm I have permission to extract data from this device (consent granted).", key='extraction_consent_checkbox')
            if agree:
...
def require_adb_ready_or_abort(st):
    if not st.session_state.get("adb_consent_ok", False):
        st.error("Cannot run adapter: device not authorized. Please accept ADB prompt on the device and press 'Re-check device'.")
        st.stop()
...
def require_adb_ui_with_override(st):
    ...
    show_dev = st.checkbox("Show developer options (advanced)", key="dev_options_toggle")
    if show_dev:
        st.markdown("**Developer override:** skip the extraction consent checkbox (unsafe). Use only in dev/testing." )
        # Option A: environment / secret based override
        override_key = st.text_input("Enter override key to enable skip (keeps session state):","", type="password", key='dev_override_key_input')
        ...
        if override_key and override_key == REAL_OVERRIDE_KEY:
            st.success("Developer override accepted — extraction consent bypass enabled for this session.")
            st.session_state.extraction_consent_given = True
            st.session_state.adb_consent_ok = True
        elif override_key:
