...
class Adapter(AdapterBase):
    ...
    def probe(self):
        ...
        try:
            ...
            if not ideviceinfo:
                return {"ok": False, "error": "libimobiledevice tools not found (ideviceinfo) - running in simulated mode"}
            out = subprocess.check_output(["idevice_id", "-l"], stderr=subprocess.STDOUT).decode()
    ...
    def extract(self, artifact_list, out_dir, metadata):
        ...
        if shutil.which("idevicebackup2") is None:
            ...
            (sim_dir / "Contacts.vcf").write_text("BEGIN:VCARD\nFN:Alice\nTEL:+1111111\nEND:VCARD\n")
            (sim_dir / "Messages.json").write_text(json.dumps([{"from":"+1111111","body":"Hi"}]))
            summary["extracted"].extend([str(Path("simulated") / "Contacts.vcf"), str(Path("simulated") / "Messages.json")])
            return summary
        try:
