...
try:
    ...
    with tabs_top[1]:
        # --- ADB diagnostic panel (inserted) ---
        import shutil, subprocess, shlex, os, sys
        st.markdown('#### ADB diagnostics')
        st.write('ADB executable in PATH:', shutil.which('adb'))
        col_a, col_b = st.columns([2,3])
        with col_a:
            if st.button('adb kill-server', key='__adb_kill'):
                try:
                    out = subprocess.run(['adb','kill-server'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=10, text=True)
                    st.code(out.stdout or '<no output>', language='bash')
                except Exception as e:
                    st.error('Failed to run adb kill-server: ' + str(e))
            if st.button('adb start-server', key='__adb_start'):
                try:
                    out = subprocess.run(['adb','start-server'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=10, text=True)
                    st.code(out.stdout or '<no output>', language='bash')
                except Exception as e:
                    st.error('Failed to run adb start-server: ' + str(e))
        with col_b:
            if st.button('adb devices', key='__adb_devices'):
                try:
                    out = subprocess.run(['adb','devices'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=10, text=True)
                    st.code(out.stdout or '<no output>', language='bash')
                    if 'unauthorized' in (out.stdout or ''):
                        st.warning('Found device in "unauthorized" state. Check your phone for the "Allow USB debugging" prompt and accept it.')
                except Exception as e:
                    st.error('Failed to run adb devices: ' + str(e))
            if st.button('adb devices -l', key='__adb_devices_l'):
                try:
                    out = subprocess.run(['adb','devices','-l'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=10, text=True)
                    st.code(out.stdout or '<no output>', language='bash')
                except Exception as e:
                    st.error('Failed to run adb devices -l: ' + str(e))
        try:
            import os as _os
            st.text_area('Environment PATH (truncated)', _os.environ.get('PATH','')[:4000], height=120)
        except Exception:
            pass
        st.info('If adb is not found, run Streamlit from the same shell where "adb devices" works or add the platform-tools folder to the system PATH. On Windows ensure drivers are installed and phone is set to File Transfer (MTP).')
        # --- end ADB diagnostic panel ---
