...
try:
    ...
    with tabs_top[1]:
            ...
            if st.button("adb devices", key="__adb_devices"):
                try:
                    ...
                    if "unauthorized" in (out.stdout or ""):
                        st.warning(
                            'Found device in "unauthorized" state. Check your phone for the "Allow USB debugging" prompt and accept it.'
                        )
                except Exception as e:
        ...
        try:
            ...
            st.text_area(
                "Environment PATH (truncated)",
                _os.environ.get("PATH", "")[:4000],
                height=120,
            )
        except Exception:
            pass
        st.info(
            'If adb is not found, run Streamlit from the same shell where "adb devices" works or add the platform-tools folder to the system PATH. On Windows ensure drivers are installed and phone is set to File Transfer (MTP).'
        )
