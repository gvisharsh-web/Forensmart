...
def run_cmd(cmd):
    try:
        cp = subprocess.run(cmd, capture_output=True, text=True, shell=False, timeout=10)
...
def detect_adb():
    ...
    return {"found": True, "version": r.get("out",""), "devices": devices.get("out",""), "raw": devices}
...
def detect_mtp_windows():
    ...
    try:
        cmd = ["powershell", "-Command", "Get-PnpDevice -Status OK | Where-Object { $_.FriendlyName -like '*MTP*' -or $_.FriendlyName -like '*Android*' } | Select-Object -Property FriendlyName,InstanceId | ConvertTo-Json"]
...
def detect_disks():
    ...
    if platform.system().lower() == "windows":
        try:
            cp = subprocess.run(["wmic", "diskdrive", "get", "model,serialnumber,caption"], capture_output=True, text=True, timeout=8)
            return {"platform":"windows", "out": cp.stdout.strip()}
        except Exception as e:
            return {"platform":"windows", "err": str(e)}
    else:
