...
# --- Consent session recovery safeguard (added) ---
import glob, json, os, pathlib
try:
    if "consent_id" not in st.session_state or not st.session_state.get("consent_id"):
        # Try to auto-recover last known consent_id from stored records
        candidates = []
        if os.path.isdir("artifacts"):
            for p in pathlib.Path("artifacts").rglob("consent.json"):
                candidates.append(p)
        if not candidates and os.path.isdir("consent_records"):
            for p in pathlib.Path("consent_records").rglob("consent.json"):
                candidates.append(p)
        if candidates:
            candidates = sorted(candidates, key=lambda p: p.stat().st_mtime)
            latest = candidates[-1]
            try:
                with open(latest, "r", encoding="utf-8") as lf:
                    data = json.load(lf)
                cid = data.get("consent_id") or data.get("consentID") or data.get("id")
                if cid:
                    st.session_state["consent_id"] = cid
                    st.info(f"Recovered consent_id from previous session: {cid}")
            except Exception as e:
                st.warning(f"Failed to recover consent_id from {latest}: {e}")
except Exception as e:
    # safeguard must not break startup
    try:
        st.warning("Consent autorecover safeguard encountered an error: " + str(e))
    except Exception:
        pass
# --- end safeguard ---

try:
    tabs_top = st.tabs(["Main App", "Adapters", "Consent"])
    ...
    with tabs_top[1]:
        ...
        # Consent verification for Android
        if consent_id:
            ok, msg = verify_consent_id(consent_id)
            if ok:
                st.success(f"Consent Verified ✅ ({consent_id})")
            else:
                st.error(f"Consent Invalid ❌ — {msg}")
        else:
            st.warning("No consent_id found. Capture consent before using adapters.")
