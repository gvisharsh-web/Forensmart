...
def render_adapter_manager_v2():
    ...
    st.write(
        "Create, view, edit adapter specifications. Detect connected devices and choose one from the list to autofill fields."
    )
    ...
    with cols[0]:
        ...
        if not adb:
            st.warning(
                "adb not found in PATH on the host. Install adb to detect Android devices."
            )
        else:
            try:
                p = subprocess.run(
                ...
                lines = [
                    ...
                    if l.strip() and not l.strip().startswith("List of devices")
                ]
                for l in lines:
                    parts = l.split()
                    if parts:
                ...
                if not android_devices:
                    ...
                    if sel:
                        ...
                        try:
                            pser = subprocess.run(
                                [adb, "-s", sel, "shell", "getprop", "ro.serialno"],
                                capture_output=True,
                                text=True,
                                timeout=5,
                            )
                        ...
                        st.info(
                            "Prefilled adapter form values from selected Android device. Open 'Adapters' page form to review and save."
                        )
    ...
    with cols[1]:
        ...
        if not idevice_id:
            st.warning(
                "idevice_id not found in PATH. Install libimobiledevice to detect iOS devices."
            )
        else:
            try:
                p = subprocess.run(
                ...
                if not ios_devices:
                    ...
                    if sel:
                        ...
                        try:
                            if ideviceinfo:
                                p2 = subprocess.run(
                                    [ideviceinfo, "-u", sel, "-k", "ProductType"],
                                    capture_output=True,
                                    text=True,
                                    timeout=10,
                                )
                        ...
                        st.info(
                            "Prefilled adapter form values from selected iOS device. Open 'Adapters' page form to review and save."
                        )
            except Exception as e:
    ...
    with st.form(key="adapter_spec_form_page", clear_on_submit=False):
        adapter_name = st.text_input(
            "Adapter name (unique)",
            value=st.session_state.get("adapter_name_prefill", "dev_adapter"),
        )
        device_type = st.selectbox(
            "Device type",
            ["android", "ios", "hdd", "other"],
            index=(
                0
                if st.session_state.get("device_type_prefill", "android") == "android"
                else 1
            ),
        )
        device_model = st.text_input(
            "Device model",
            value=st.session_state.get("device_model_prefill", "Unknown Model"),
        )
        device_serial = st.text_input(
            "Device serial / id",
            value=st.session_state.get("device_serial_prefill", ""),
        )
        note = st.text_area(
            "Note / description", value=st.session_state.get("note_prefill", "")
        )
    ...
    if save_btn:
        ...
        if _adp_mod:
            try:
                ...
                if ok:
                    st.success(
                        f"Saved adapter spec for '{adapter_name}' to adapters storage."
                    )
                else:
                    ...
                    with st.expander(k):
                        ...
                        if st.button(f"Delete '{k}'", key=f"del_spec_{k}"):
                            try:
                                ...
                                if k in data:
                                    del data[k]
                                    _adp_mod2.ADAPTER_SPECS_FILE.write_text(
                                        json.dumps(data, indent=2), encoding="utf-8"
                                    )
                                    st.success(
                                        f"Deleted spec '{k}'. Please re-open Adapters page to refresh."
                                    )
                                else:
...
def render_adapter_manager_v2():
    ...
    st.write(
        "Create, view, edit adapter specifications. You can detect a connected device to autofill fields (Android/iOS detection attempted)."
    )
        ...
        if st.button("Detect connected Android device (adb)"):
            ...
            if not adb:
                ...
                try:
                    ...
                    if not lines:
                        st.warning(
                            "No adb devices found. Ensure USB debugging enabled and device connected."
                        )
                    else:
                        ...
                        try:
                            pser = subprocess.run(
                                [adb, "-s", first, "shell", "getprop", "ro.serialno"],
                                capture_output=True,
                                text=True,
                                timeout=5,
                            )
                        ...
                        st.success(
                            "Prefilled adapter form values from connected Android device. Open 'Adapters' page form to review and save."
                        )
    ...
    with cols[1]:
        if st.button("Detect connected iOS device (idevice_id)"):
            ...
            if not idevice_id:
                st.error(
                    "idevice_id not found in PATH. Install libimobiledevice and retry."
                )
            else:
                try:
                    p = subprocess.run(
                    ...
                    if not lines:
                        ...
                        try:
                            if ideviceinfo:
                                p2 = subprocess.run(
                                    [ideviceinfo, "-u", first, "-k", "ProductType"],
                                    capture_output=True,
                                    text=True,
                                    timeout=10,
                                )
                        ...
                        st.success(
                            "Prefilled adapter form values from connected iOS device. Open 'Adapters' page form to review and save."
                        )
                except Exception as e:
                    st.error("Error while running idevice tools: " + str(e))
...
try:
                    ...
                    if not consent_id:
                        ...
                        try:
                            runner = AdapterRunner(
                                adapter_module="adapters.android_adb",
                                adapter_cls="Adapter",
                            )
        ...
        if verifier.exists():
            try:
                ...
                if cp.returncode == 0:
                    ...
                    return (
                        False,
                        f"Verifier reported failure: {cp.stdout}\\n{cp.stderr}",
                    )
            except Exception as e:
            ...
            if has_hash and has_sig:
                return (
                    True,
                    "Basic presence checks OK (artifact_hash + signature present).",
                )
    ...
    with st.sidebar.expander("🔌 Data Adapters & Consent", expanded=True):
        ...
        if st.session_state.get("adapters_last_sync"):
            st.write(
                "Last adapters save (UTC):", st.session_state["adapters_last_sync"]
            )
        ...
        if st.button("Record Consent"):
            ...
            try:
                ...
                cid = hashlib.sha256(
                    json.dumps(
                        st.session_state["consent_record"], sort_keys=True
                    ).encode()
                ).hexdigest()
        ...
        def export_unified_case_report(metadata):
            ...
            if "suspicious_messages" in st.session_state:
                ...
                pdf.multi_cell(
                    0,
                    8,
                    f"Hash: {compute_hash(st.session_state['suspicious_messages'])}",
                )
                pdf.ln(5)
    ...
    with tab1:
        ...
        if mode == "USB GPS Logs":
            gps_file = st.file_uploader(
                "Upload GPS data (CSV with timestamp, latitude, longitude)",
                type=["csv"],
            )
            gps_df = None
            if gps_file:
                ...
                if "latitude" in gps_df.columns and "longitude" in gps_df.columns:
                    ...
                    st.error(
                        "GPS CSV must include latitude/longitude or lat/lon columns."
                    )
                    gps_df = None
            ...
            if gps_df is not None and not gps_df.empty:
                ...
                if crime_file:
                    ...
                    if crime_df is not None and not crime_df.empty:
                        if {"latitude", "longitude"}.issubset(
                            set(crime_df.columns)
                        ) or {"lat", "lon"}.issubset(set(crime_df.columns)):
                        ...
                        if st.button("➕ Add manual hotspot"):
                            ...
                            if over:
                                st.markdown(
                                    "### 🚨 Overlaps with manual hotspots (≤200m)"
                                )
                ...
                if output and "all_drawings" in output and output["all_drawings"]:
                    ...
                    st.image(
                        tmpf.name, caption="Crime zone snapshot (saved for report)"
                    )
                    if matches:
        ...
        # ---------------- Manual Input ----------------
            ...
            sub = st.radio(
                "Manual input type", ["Add Coordinate", "Enter Timestamp Only"]
            )
            if sub == "Add Coordinate":
                ...
                if st.button("➕ Add as manual point"):
                    ...
                    st.map(
                        pd.DataFrame(st.session_state["manual_points"])[["lat", "lon"]]
                    )
            else:
    ...
    with tab6:
        ...
        abstract = st.text_area(
            "Write a short case abstract / summary (this goes on cover page)",
            height=120,
        )
        ...
        def export_unified_report_pdf():
            ...
            if heatmap_path and os.path.exists(heatmap_path):
                ...
                try:
                    ...
                    pdf.multi_cell(
                        0, 7, f"[Heatmap image available at: {heatmap_path}]"
                    )
                pdf.ln(4)
            ...
            if "crime_hotspots_manual" in st.session_state:
                ...
                try:
                    ...
                    pdf.multi_cell(
                        0, 6, "Manual hotspots present but failed to render."
                    )
                pdf.ln(2)
                pdf.set_font("Arial", "I", 9)
                pdf.multi_cell(
            ...
            if "crime_zones" in st.session_state:
                ...
                pdf.multi_cell(
                    0,
                    6,
                    f"Hash (SHA256) of drawn zones object: {compute_hash(zones_obj)}",
                )
                pdf.ln(4)
            ...
            if "link_points" in st.session_state:
                ...
                pdf.multi_cell(
                    0,
                    6,
                    f"Hash (SHA256): {st.session_state.get('link_points_hash','N/A')}",
                )
                pdf.ln(4)
            ...
            if "cell_tower_points" in st.session_state:
                ...
                try:
                    ...
                    pdf.multi_cell(
                        0, 6, "Cell tower data present but failed to render."
                    )
                pdf.ln(2)
                pdf.set_font("Arial", "I", 9)
                pdf.multi_cell(
                    0,
                    6,
                    f"Hash (SHA256): {st.session_state.get('cell_tower_hash','N/A')}",
                )
                pdf.ln(4)
            ...
            if "suspicious_messages" in st.session_state:
                ...
                try:
                    for m in st.session_state.get("suspicious_messages", []):
                        ...
                        score = m.get(
                            "suspicion_score",
                            m.get("score", m.get("offense_score", 0.0)),
                        )
                        pdf.multi_cell(
            ...
            if os.path.exists(parsed_dir):
                try:
                    ...
                    if not files:
                        pdf.multi_cell(
                            0, 6, "No parsed files found in parsed directory."
                        )
                    else:
                        for fn in files:
    ...
    with st.expander("Comms Analyzer — Run / Utilities", expanded=True):
        ...
        if st.button("Run Comms Analyzer Pipeline"):
            ...
            for t in texts:
                ...
                lang_counts[lang] += 1
            dominant_lang = (
                lang_counts.most_common(1)[0][0] if lang_counts else "unknown"
            )
    ...
    with st.expander(
        ...
        if "susp_label_index" not in st.session_state:
            ...
            st.session_state["susp_labels"] = (
                []
            )  # list of dicts {text,label,sender,time}
        if len(msgs) == 0:
            ...
            with nav3:
                if st.button("Clear Labels", key="susp_clear"):
                    st.session_state["susp_labels"] = []
                    st.success(
                        "Cleared temporary labels (will not affect saved model)."
                    )
        ...
        if st.button("Train/Update Suspicious Classifier"):
            ...
            if lab_df.shape[0] < 10:
                st.warning(
                    "Please label at least 10 messages (balanced) before training."
                )
            else:
    ...
    with st.expander("Apply Suspicious Classifier / Heuristics", expanded=True):
        ...
        if st.button("Run Suspicion Scoring (Classifier + Heuristics)"):
            ...
            # sort by score desc
            suspicious_out = sorted(
                suspicious_out, key=lambda x: x["score"], reverse=True
            )
    ...
    def save_transformer_model(
        model, tokenizer, out_dir="models/suspicious_transformer"
    ):
    ...
    def train_transformer_classifier(
        ...
        def compute_metrics(eval_pred):
            ...
            return {
                ...
                "precision": float(
                    precision_score(labels_eval, preds, zero_division=0)
                ),
    ...
    # ---------------------------
    with st.expander(
        "Upgraded Suspicious Classifier (Train / Evaluate)", expanded=True
    ):
        st.write(
        ...
        if df_train is None or df_train.empty:
            ...
            if model_choice == "tfidf":
                ...
                out_dir = st.text_input(
                    "Output dir", value="models/suspicious_transformer"
                )
    ...
    # ---------------------------
    with st.expander(
        "Apply Suspicious Model to Messages (Run Inference)", expanded=True
    ):
        st.write(
    ...
    with st.expander(
        ...
        with cols[0]:
            n_rounds = st.number_input(
                "Rounds", min_value=1, max_value=10, value=3, step=1
            )
        with cols[1]:
            ...
            if len(train_texts) < 1:
                ...
                if model is not None:
                    try:
                        ...
                        st.session_state["susp_pseudo_added_count"] = len(
                            all_new_pseudo
                        )
        ...
        if not pool:
            ...
            for idx in sample:
                ...
                if choice != "Unlabeled":
                    ...
                    new_pool = [
                        p
                        for p in st.session_state.get("susp_active_pool", [])
                        if p != idx
                    ]
    ...
    with st.expander("Export / Inspect semi-supervised artifacts", expanded=False):
        if st.button("Show indices of pseudo-labeled examples (session)"):
            st.write(
                "Pseudo-added-count:",
                st.session_state.get("susp_pseudo_added_count", 0),
            )
...
def render_overview():
    ...
    st.write(
        "This single-entry app integrates Social Parsers, Artifacts management and Location Intelligence for testing."
    )
...
def render_artifacts_manager():
    ...
    st.write(
        "View and manage artifacts pulled from devices. Files are stored under `/mnt/data/pulled_data/`. Use delete to remove artifacts when done."
    )
    ...
    if not base.exists():
        ...
        return
    entries = sorted(
        [p for p in base.iterdir() if p.is_dir()], key=lambda x: x.name, reverse=True
    )
    for e in entries:
        with st.expander(e.name, expanded=False):
            ...
            if key_move not in st.session_state:
                if cols[0].button(
                    "Move this artifact (soft-delete)", key=f"del_{e.name}"
                ):
                    st.session_state[key_move] = True
            else:
                cols[0].warning("Confirm moving artifact: " + e.name)
                if cols[0].checkbox(
                    "I confirm moving this artifact to /tmp/forensmart_deleted",
                    key=f"chk_move_{e.name}",
                ):
            ...
            if key_del not in st.session_state:
                if cols[1].button(
                    "Permanently delete (irreversible)", key=f"rm_{e.name}"
                ):
                    st.session_state[key_del] = True
            else:
                cols[1].warning("Permanent delete confirmation required for: " + e.name)
                if cols[1].checkbox(
                    "I understand this is irreversible", key=f"chk_del_{e.name}"
                ):
                    if cols[1].button(
                        "Confirm Permanent Delete", key=f"confirm_del_btn_{e.name}"
                    ):
                        try:
                            if e.exists():
                                shutil.rmtree(e)
            ...
            if cols[2].button("Download as ZIP", key=f"zip_{e.name}"):
                ...
                try:
                    shutil.make_archive(
                        str(zip_path).replace(".zip", ""), "zip", root_dir=str(e)
                    )
...
def render_tools_installer():
    ...
    st.write(
        "Manage forensic tool dependencies. This installer will create placeholder tool files under /mnt/data/tools to simulate installation."
    )
    ...
    for name, meta in candidates.items():
        ...
        if col2.button(f"Install {name}", key=f"install_{name}"):
            try:
                tool_file.write_text(
                    json.dumps({"name": name, "installed_at": int(time.time())}),
                    encoding="utf-8",
                )
        ...
        if col3.button(f"Update {name}", key=f"update_{name}"):
            if installed:
                tool_file.write_text(
                    json.dumps({"name": name, "updated_at": int(time.time())}),
                    encoding="utf-8",
                )
    ...
    st.info(
        "Note: This installer simulates installs by creating placeholder files. Replace with real installers/downloads for production."
    )
