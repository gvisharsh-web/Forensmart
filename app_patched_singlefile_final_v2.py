...
# --- Compatibility shim: ensure column objects support 'with' context manager ---
def _ensure_col_context(col):
    # If column already supports context manager, return as-is
...
try:
    ...
    def _columns_wrapped(*a, **k):
        cols = _orig_columns(*a, **k)
        try:
            return [_ensure_col_context(c) for c in cols]
        except Exception:
            return cols
    st.columns = _columns_wrapped
except Exception:
    pass
...
st.set_page_config(page_title='Forensmart - Single Entry Dev App', layout='wide')
st.sidebar.title("Forensmart - Single Entry")


def render_adapter_manager_v2():
    st.title("Adapters — Manage saved adapter specs (Device Selection)")
    st.write("Create, view, edit adapter specifications. Detect connected devices and choose one from the list to autofill fields.")

    try:
        import adapters as _adp_mod
    except Exception:
        _adp_mod = None

    cols = st.columns([2,2])
    with cols[0]:
        st.write("Android device detection")
        adb = shutil.which("adb")
        android_devices = []
        if not adb:
            st.warning("adb not found in PATH on the host. Install adb to detect Android devices.")
        else:
            try:
                p = subprocess.run([adb, "devices"], capture_output=True, text=True, timeout=10)
                lines = [l.strip() for l in p.stdout.splitlines() if l.strip() and not l.strip().startswith("List of devices")]
                for l in lines:
                    parts = l.split()
                    if parts:
                        android_devices.append(parts[0])
                if not android_devices:
                    st.info("No Android devices detected via adb.")
                else:
                    sel_idx = 0
                    sel = st.selectbox("Select Android device", options=android_devices, index=0, key="android_select_box")
                    if sel:
                        st.success(f"Selected device: {sel}")
                        # try to read model and serial for selected device
                        try:
                            pmod = subprocess.run([adb, "-s", sel, "shell", "getprop", "ro.product.model"], capture_output=True, text=True, timeout=5)
                            model = pmod.stdout.strip()
                        except Exception:
                            model = ""
                        try:
                            pser = subprocess.run([adb, "-s", sel, "shell", "getprop", "ro.serialno"], capture_output=True, text=True, timeout=5)
                            serial = pser.stdout.strip() or sel
                        except Exception:
                            serial = sel
                        st.session_state['adapter_name_prefill'] = f"android_{serial}"
                        st.session_state['device_type_prefill'] = "android"
                        st.session_state['device_model_prefill'] = model or ""
                        st.session_state['device_serial_prefill'] = serial or sel
                        st.info("Prefilled adapter form values from selected Android device. Open 'Adapters' page form to review and save.")
            except Exception as e:
                st.error("Error while running adb: " + str(e))

    with cols[1]:
        st.write("iOS device detection")
        idevice_id = shutil.which("idevice_id") or shutil.which("idevice_id.exe")
        ideviceinfo = shutil.which("ideviceinfo") or shutil.which("ideviceinfo.exe")
        ios_devices = []
        if not idevice_id:
            st.warning("idevice_id not found in PATH. Install libimobiledevice to detect iOS devices.")
        else:
            try:
                p = subprocess.run([idevice_id, "-l"], capture_output=True, text=True, timeout=10)
                lines = [l.strip() for l in p.stdout.splitlines() if l.strip()]
                for l in lines:
                    ios_devices.append(l.strip())
                if not ios_devices:
                    st.info("No iOS devices found via idevice_id -l.")
                else:
                    sel = st.selectbox("Select iOS device (UDID)", options=ios_devices, index=0, key="ios_select_box")
                    if sel:
                        st.success(f"Selected iOS UDID: {sel}")
                        model = ""
                        try:
                            if ideviceinfo:
                                p2 = subprocess.run([ideviceinfo, "-u", sel, "-k", "ProductType"], capture_output=True, text=True, timeout=10)
                                model = p2.stdout.strip()
                        except Exception:
                            model = ""
                        st.session_state['adapter_name_prefill'] = f"ios_{sel}"
                        st.session_state['device_type_prefill'] = "ios"
                        st.session_state['device_model_prefill'] = model or ""
                        st.session_state['device_serial_prefill'] = sel
                        st.info("Prefilled adapter form values from selected iOS device. Open 'Adapters' page form to review and save.")
            except Exception as e:
                st.error("Error while running idevice tools: " + str(e))

    st.markdown('---')
    # The interactive form uses session_state prefill values if present
    with st.form(key="adapter_spec_form_page", clear_on_submit=False):
        adapter_name = st.text_input("Adapter name (unique)", value=st.session_state.get('adapter_name_prefill','dev_adapter'))
        device_type = st.selectbox("Device type", ["android", "ios", "hdd", "other"], index=0 if st.session_state.get('device_type_prefill','android')=='android' else 1)
        device_model = st.text_input("Device model", value=st.session_state.get('device_model_prefill','Unknown Model'))
        device_serial = st.text_input("Device serial / id", value=st.session_state.get('device_serial_prefill',''))
        note = st.text_area("Note / description", value=st.session_state.get('note_prefill',''))
        save_btn = st.form_submit_button("Save adapter spec")
    if save_btn:
        spec = {
        ...
        if _adp_mod:
            try:
                ok = _adp_mod.set_adapter_spec(adapter_name, spec)
                if ok:
                    st.success(f"Saved adapter spec for '{adapter_name}' to adapters storage.")
                else:
                    st.error("Failed to save adapter spec (write error).")
            except Exception as e:
                st.error("Error saving adapter spec: " + str(e))
        else:
            st.warning("adapters module not available; spec not saved to disk.")

    # Viewer: list current saved adapters
    try:
        import adapters as _adp_mod2
    except Exception:
        _adp_mod2 = None
    try:
        if _adp_mod2:
            st.write("Saved adapter specs:")
            s = _adp_mod2.load_adapter_specs() or {}
            if s:
                for k, v in s.items():
                    with st.expander(k):
                        st.json(v)
                        if st.button(f"Delete '{k}'", key=f"del_spec_{k}"):
                            try:
                                data = _adp_mod2.load_adapter_specs()
                                if k in data:
                                    del data[k]
                                    _adp_mod2.ADAPTER_SPECS_FILE.write_text(json.dumps(data, indent=2), encoding='utf-8')
                                    st.success(f"Deleted spec '{k}'. Please re-open Adapters page to refresh.")
                                else:
                                    st.info("Spec not found.")
                            except Exception as e:
                                st.error("Failed to delete spec: " + str(e))
            else:
                st.info("No adapter specs saved yet.")
    except Exception:
        pass
...
def render_adapter_manager_v2():
    st.title("Adapters — Manage saved adapter specs")
    st.write("Create, view, edit adapter specifications. You can detect a connected device to autofill fields (Android/iOS detection attempted).")
    # Attempt to import adapters module
    try:
        import adapters as _adp_mod
    except Exception:
        _adp_mod = None
    ...
    cols = st.columns([2,2])
    with cols[0]:
        if st.button("Detect connected Android device (adb)"):
            # Try to detect via adb
            adb = shutil.which("adb")
            if not adb:
                st.error("adb not found in PATH on the host. Install adb and retry.")
            else:
                try:
                    p = subprocess.run([adb, "devices"], capture_output=True, text=True, timeout=10)
                    lines = [l.strip() for l in p.stdout.splitlines() if l.strip() and not l.strip().startswith("List of devices")]
                    if not lines:
                        st.warning("No adb devices found. Ensure USB debugging enabled and device connected.")
                    else:
                        # pick first device line
                        first = lines[0].split()[0]
                        st.success(f"Found device: {first}")
                        # try to read model and serial via getprop
                        try:
                            pmod = subprocess.run([adb, "-s", first, "shell", "getprop", "ro.product.model"], capture_output=True, text=True, timeout=5)
                            model = pmod.stdout.strip()
                        except Exception:
                            model = ""
                        try:
                            pser = subprocess.run([adb, "-s", first, "shell", "getprop", "ro.serialno"], capture_output=True, text=True, timeout=5)
                            serial = pser.stdout.strip()
                        except Exception:
                            serial = first
                        # Save prefill values into session_state
                        st.session_state['adapter_name_prefill'] = f"android_{serial}"
                        st.session_state['device_type_prefill'] = "android"
                        st.session_state['device_model_prefill'] = model or ""
                        st.session_state['device_serial_prefill'] = serial or first
                        st.success("Prefilled adapter form values from connected Android device. Open 'Adapters' page form to review and save.")

                except Exception as e:
                    st.error("Error while running adb: " + str(e))

    with cols[1]:
        if st.button("Detect connected iOS device (idevice_id)"):
            # Try idevice_id and ideviceinfo
            idevice_id = shutil.which("idevice_id") or shutil.which("idevice_id.exe")
            ideviceinfo = shutil.which("ideviceinfo") or shutil.which("ideviceinfo.exe")
            if not idevice_id:
                st.error("idevice_id not found in PATH. Install libimobiledevice and retry.")
            else:
                try:
                    p = subprocess.run([idevice_id, "-l"], capture_output=True, text=True, timeout=10)
                    lines = [l.strip() for l in p.stdout.splitlines() if l.strip()]
                    if not lines:
                        st.warning("No iOS devices found via idevice_id -l.")
                    else:
                        first = lines[0].strip()
                        st.success(f"Found iOS device UDID: {first}")
                        model = ""
                        try:
                            if ideviceinfo:
                                p2 = subprocess.run([ideviceinfo, "-u", first, "-k", "ProductType"], capture_output=True, text=True, timeout=10)
                                model = p2.stdout.strip()
                        except Exception:
                            model = ""
                        st.session_state['adapter_name_prefill'] = f"ios_{first}"
                        st.session_state['device_type_prefill'] = "ios"
                        st.session_state['device_model_prefill'] = model or ""
                        st.session_state['device_serial_prefill'] = first
                        st.success("Prefilled adapter form values from connected iOS device. Open 'Adapters' page form to review and save.")
                except Exception as e:
                    st.error("Error while running idevice tools: " + str(e))

    st.markdown('---')
    # The interactive form (same as before) but using session_state prefill values if present
    with st.form(key="adapter_spec_form_page", clear_on_submit=False):
        adapter_name = st.text_input("Adapter name (unique)", value=st.session_state.get('adapter_name_prefill','dev_adapter'))
        device_type = st.selectbox("Device type", ["android", "ios", "hdd", "other"], index=0 if st.session_state.get('device_type_prefill','android')=='android' else 1)
        device_model = st.text_input("Device model", value=st.session_state.get('device_model_prefill','Unknown Model'))
...
def render_dev_mode_ui():
    # Adapter Spec management moved to the 'Adapters' page in the sidebar.
    pass

def _auto_store_adapter_spec_demo():
    try:
        import adapters as _adp
        demo = {
            "last_seen": int(time.time()),
            "note": "auto-saved adapter spec from singlefile integration",
            "device_hint": None
        }
        _adp.set_adapter_spec("dev_auto", demo)
    except Exception:
        pass

# call once on dev UI render to ensure adapters file is used for specs storage
try:
    _auto_store_adapter_spec_demo()
except Exception:
    pass
    ...
    with st.sidebar.expander("🔌 Data Adapters & Consent", expanded=True):
        st.markdown("Configure data adapters (ingest connectors) and capture investigator consent / chain-of-custody notes.")
        AVAILABLE_ADAPTERS = [
            "Local CSV", "Google Drive (link)", "iCloud (link)", "ADB (USB)", "WhatsApp Export (.txt/.zip)", "Telegram Export",
            "Manual JSON", "SFTP"
        ]
        ...
        sel = st.multiselect("Select active adapters", AVAILABLE_ADAPTERS, default=st.session_state.get("selected_adapters", []))
        cols = st.columns([3,1])
        with cols[0]:
            adapter_notes = st.text_area("Adapter notes / endpoint config (JSON or plain text)", value=st.session_state.get("adapter_config", {}).get("notes", ""), height=80)
        with cols[1]:
            if st.button("Save Adapters"):
                prev = set(st.session_state.get("selected_adapters", []))
                new = set(sel)
                st.session_state["adapter_config"] = {"notes": adapter_notes, "saved_at": str(datetime.datetime.utcnow())}
                st.session_state["selected_adapters"] = list(sel)
                st.session_state["adapters_last_sync"] = datetime.datetime.utcnow().isoformat() + "Z"
                if prev != new:
                    st.session_state["adapters_discrepancy"] = True
                    st.warning("Adapter set changed — discrepancy flagged. Please review ingestion steps.")
                else:
        ...
        if st.session_state.get("adapters_discrepancy", False):
            st.error("⚠️ Adapter discrepancy detected. Consider re-running extraction or recording justification below.")
            justification = st.text_area("Discrepancy justification (investigator notes):", value=st.session_state.get("adapter_config", {}).get("discrepancy_just", ""), height=80)
        ...
        if "consent_record" not in st.session_state:
            st.session_state["consent_record"] = {"consent_given": False, "by": None, "timestamp": None, "notes": None}
        ccol1, ccol2 = st.columns([3,2])
        with ccol1:
            consent = st.checkbox("I confirm I have authorization to analyze this data (Give Consent)", value=st.session_state["consent_record"].get("consent_given", False))
        with ccol2:
            initials = st.text_input("Investigator initials", value=st.session_state["consent_record"].get("by") or "")
        consent_notes = st.text_area("Consent / custody notes (optional)", value=st.session_state["consent_record"].get("notes", ""), height=80)
        if st.button("Record Consent"):
            ...
            try:
                cr_dir = os.path.join("consent_records", st.session_state.get("case_id","unknown_case"))
                ...
                cid = hashlib.sha256(json.dumps(st.session_state["consent_record"], sort_keys=True).encode()).hexdigest()
    ...
    with tab6:
        ...
        def export_unified_report_pdf():
            ...
            pdf.set_font("Arial", size=11)
            parsed_dir = os.path.join("uploads", st.session_state.get("case_id", "unknown_case"), "parsed")
            if os.path.exists(parsed_dir):
                try:
                    files = sorted([f for f in os.listdir(parsed_dir) if os.path.isfile(os.path.join(parsed_dir, f))])
                    if not files:
                        ...
                        for fn in files:
                            ...
                            try:
                                ...
                                if isinstance(data, dict) and 'messages' in data:
                                    pdf.set_font("Arial", size=10)
                                    pdf.multi_cell(0, 6, '  Messages parsed: ' + str(len(data.get("messages", []))))
    
                                    for sm in data.get('messages', [])[:3]:
                                        txt = sm.get('text') if isinstance(sm, dict) else str(sm)
                                        pdf.multi_cell(0, 6, '    • ' + (txt[:200] if txt else ''))
                                else:
                                    preview = json.dumps(data)[:500]
                                    pdf.set_font("Arial", size=10)
                                    pdf.multi_cell(0, 6, '  Preview: ' + preview)
                            except Exception:
                                pdf.multi_cell(0, 6, '  [Failed to load parsed file preview]')
                            try:
                    ...
                    pdf.multi_cell(0, 6, 'Parsed directory exists but failed to enumerate files.')
            else:
        ...
        if st.button("📄 Generate Unified PDF Report"):
            ...
            if consent_id and 'finalize_report_integrity' in globals():
                try:
                    updated_pdf, master_hash = finalize_report_integrity(consent_id, out_bytesio.getvalue(), investigator)
                    # replace out_bytesio content if applicable (store hash in session)
                    st.session_state['master_report_hash'] = master_hash
                    out_bytesio = io.BytesIO(updated_pdf) if isinstance(updated_pdf, (bytes, bytearray)) else out_bytesio
    ...
    def parse_whatsapp_db(db_path):
        ...
        try:
            ...
            if 'messages' in tables:
                try:
                    cur.execute("SELECT _id, key_remote_jid, key_from_me, data, timestamp FROM messages ORDER BY timestamp ASC")
                    rows = cur.fetchall()
                    for row in rows:
                        ...
                        msgs.append({"id": _id, "sender": jid, "from_me": from_me, "text": text, "timestamp": ts_val})
                except Exception as e:
    ...
    def auto_parse_social_media(adb_out_dir):
        ...
        for root, dirs, files in os.walk(adb_out_dir):
            for fn in files:
                if (fn.startswith("msgstore") and fn.endswith(".db")) or (fn.startswith("wa") and fn.endswith(".db")):
        ...
        if wa_dbs:
            ...
            for db in wa_dbs:
                ...
                out_dir = os.path.join('uploads', st.session_state.get('case_id','unknown_case'), 'parsed')
                ...
                if isinstance(res, dict) and 'messages' in res and res['messages']:
                    ...
                    for m in res['messages']:
                        st.session_state['chats'].append({'sender': m.get('sender'), 'time': m.get('timestamp'), 'text': m.get('text') or '', 'source_db': db})
        # Telegram: look for cache*.db or cache4.db
        tg_dbs = []
        for root, dirs, files in os.walk(adb_out_dir):
            for fn in files:
                if ('cache' in fn and fn.endswith('.db')) or ('telegram' in fn.lower() and fn.endswith('.db')):
        ...
        if tg_dbs:
            ...
            for db in tg_dbs:
                ...
                if isinstance(res, dict) and 'messages' in res and res['messages']:
                    st.session_state.setdefault('chats', [])
                    for m in res['messages']:
                        st.session_state['chats'].append({'sender': m.get('sender') if isinstance(m, dict) else None, 'time': None, 'text': m.get('text') if isinstance(m, dict) else str(m), 'source_db': db})
...
# Render functions loaded above

def render_overview():
    st.title("Forensmart - Development Single Entry (DEV MODE)")
    st.write("This single-entry app integrates Social Parsers, Artifacts management and Location Intelligence for testing.")
    st.write("- Use the sidebar to switch between modules.")


# Insert cleaned Social UI as a function that will be executed in-place
def render_social_parsers():
    # The social UI code below is cleaned to avoid duplicate top-level Streamlit configs.
    # Social UI not found
    pass


# Insert location UI rendering
def render_location_intel():
    # Location UI not found
    pass


def render_artifacts_manager():
    st.title("Pulled Artifacts Management")
    st.write("View and manage artifacts pulled from devices. Files are stored under `/mnt/data/pulled_data/`. Use delete to remove artifacts when done.")
    base = Path('/mnt/data/pulled_data')
    if not base.exists():
        st.info("No pulled artifacts found in /mnt/data/pulled_data")
        return
    entries = sorted([p for p in base.iterdir() if p.is_dir()], key=lambda x: x.name, reverse=True)
    for e in entries:
        with st.expander(e.name, expanded=False):
            st.write("Path:", str(e))
            meta = e / 'pull_artifacts_meta.json'
            if meta.exists():
                try:
                    ...
                    st.write("Could not read metadata:", ex)
            files = list(e.rglob('*'))
            st.write(f"Contains {len(files)} files (showing up to 200):")
            for f in files[:200]:
                st.write(str(f.relative_to(e)))
            cols = st.columns([1,1,1])
            # Two-step move confirmation
            key_move = f"confirm_move_{e.name}"
            if key_move not in st.session_state:
                if cols[0].button('Move this artifact (soft-delete)', key=f'del_{e.name}'):
                    st.session_state[key_move] = True
            else:
                cols[0].warning('Confirm moving artifact: ' + e.name)
                if cols[0].checkbox('I confirm moving this artifact to /tmp/forensmart_deleted', key=f'chk_move_{e.name}'):
                    if cols[0].button('Confirm Move', key=f'confirm_move_btn_{e.name}'):
                        target = Path('/tmp/forensmart_deleted') / e.name
                        target.parent.mkdir(parents=True, exist_ok=True)
                        try:
                            shutil.move(str(e), str(target))
                            st.success(f"Moved {e.name} to {target}")
                            del st.session_state[key_move]
                        except Exception as ex:
                            st.error(f"Failed to move: {ex}")
            # Two-step permanent delete
            key_del = f"confirm_del_{e.name}"
            if key_del not in st.session_state:
                if cols[1].button('Permanently delete (irreversible)', key=f'rm_{e.name}'):
                    st.session_state[key_del] = True
            else:
                cols[1].warning('Permanent delete confirmation required for: ' + e.name)
                if cols[1].checkbox('I understand this is irreversible', key=f'chk_del_{e.name}'):
                    if cols[1].button('Confirm Permanent Delete', key=f'confirm_del_btn_{e.name}'):
                        try:
                            if e.exists():
                                shutil.rmtree(e)
                            st.success(f"Permanently deleted {e.name}")
                            del st.session_state[key_del]
                        except Exception as ex:
                            st.error(f"Failed to delete: {ex}")
            if cols[2].button('Download as ZIP', key=f'zip_{e.name}'):
                zip_path = Path('/mnt/data') / f"{e.name}.zip"
                try:
                    shutil.make_archive(str(zip_path).replace('.zip',''), 'zip', root_dir=str(e))
                    st.success(f"Zipped to {zip_path}")
                    st.write('You can download it from the server path:' , str(zip_path))
                except Exception as ex:
                    st.error(f"Failed to create zip: {ex}")



# Sidebar navigation
page = st.sidebar.selectbox("Go to", ["Overview", "Social Parsers", "Artifacts", "Location Intelligence", "Adapters", "Dev Mode"])
if page == "Overview":
    render_overview()
elif page == "Social Parsers":
    # execute social parsers UI context
    try:
        render_social_parsers()
    except Exception as e:
        st.error("Social Parsers UI failed: " + str(e))
        st.code(traceback.format_exc())

elif page == "Adapters":
    try:
        render_adapter_manager_v2()
    except Exception as e:
        st.error("Adapters page failed: " + str(e))
        st.code(traceback.format_exc())

elif page == "Artifacts":
    render_artifacts_manager()
elif page == "Location Intelligence":
    try:
        render_location_intel()
    except Exception as e:
        st.error("Location Intelligence UI failed: " + str(e))
        st.code(traceback.format_exc())
    ...
    try:
        ...
        st.error("Dev Mode UI failed: " + str(e))
        st.code(traceback.format_exc())
...
def render_tools_installer():
    st.title("Tools Installer & Updater")
    st.write("Manage forensic tool dependencies. This installer will create placeholder tool files under /mnt/data/tools to simulate installation.")
    tools_dir = Path("/mnt/data/tools")
    tools_dir.mkdir(parents=True, exist_ok=True)
    # Define candidate tools and placeholder info
    candidates = {
        "adb": {"desc": "Android Debug Bridge", "bin": "adb"},
        "ideviceinfo": {"desc": "libimobiledevice ideviceinfo", "bin": "ideviceinfo"},
        "idevice_id": {"desc": "libimobiledevice idevice_id", "bin": "idevice_id"},
        "smartctl": {"desc": "smartctl (smartmontools)", "bin": "smartctl"},
        "exiftool": {"desc": "ExifTool", "bin": "exiftool"}
    }
    st.write("Tools directory:", str(tools_dir))
    for name, meta in candidates.items():
        tool_file = tools_dir / name
        installed = tool_file.exists()
        cols = st.columns([3,1,1])
        col1, col2, col3 = cols[0], cols[1], cols[2]
        with col1:
            st.write(f"**{name}** — {meta['desc']}")
            st.write(f"Installed: {'Yes' if installed else 'No'}")
        if col2.button(f"Install {name}", key=f"install_{name}"):
            try:
                tool_file.write_text(json.dumps({"name": name, "installed_at": int(time.time())}), encoding='utf-8')
                st.success(f"Installed {name} (simulated)")
            except Exception as e:
                st.error(f"Failed to install {name}: {e}")
        if col3.button(f"Update {name}", key=f"update_{name}"):
            if installed:
                tool_file.write_text(json.dumps({"name": name, "updated_at": int(time.time())}), encoding='utf-8')
                st.success(f"Updated {name} (simulated)")
            else:
                st.warning(f"{name} not installed. Install first.")
    st.markdown('---')
    st.subheader('Prebuilt binary downloads (simulated)')
    prebuilt_dir = Path('/mnt/data/prebuilts')
    prebuilt_dir.mkdir(parents=True, exist_ok=True)
    for name in candidates.keys():
        if st.button(f"Download prebuilt {name}", key=f"dl_{name}"):
            p = prebuilt_dir / f"{name}_prebuilt.bin"
            p.write_bytes(b"PREBUILT_BINARY_PLACEHOLDER\n")
            st.success(f"Downloaded prebuilt binary to {p}")
    st.info("Note: This installer simulates installs by creating placeholder files. Replace with real installers/downloads for production.")
